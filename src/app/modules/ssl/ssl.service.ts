...
const initPayment = async (paymentData: IPaymentData) => {
  try {
    const data = {
      ...
      currency: "BDT",
      tran_id: paymentData.tranasctionId, // use unique tran_id for each api call
      success_url: config.ssl.successUrl,
      fail_url: config.ssl.failUrl,
      cancel_url: config.ssl.cancelUl,
...
const validatePayment = async (payload: any) => {
  try {
    ...
    throw new ApiError(httpStatus.BAD_REQUEST,'Payment validation faild')
  }
};
